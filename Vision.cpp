...
Vision::~Vision() {
	CommandBase::shooter->SetCameraLED(false);
...
bool Vision::hotGoal() {
	//Threshold threshold(105, 137, 230, 255, 133, 183);	//RGB threshold criteria, ranges are in that order ie. Hue is 60-100
	Threshold threshold(0, 50, 200, 255, 0, 175);	//RGB threshold criteria, ranges are in that order ie. Hue is 60-100
	//Threshold threshold(0, 75, 100, 255, 50, 200);	//RGB threshold criteria, ranges are in that order ie. Hue is 60-100
...
//	image->Write("/rawImage.bmp");
	thresholdImage = image->ThresholdRGB(threshold);	// get just the green target pixels
	delete image;
	thresholdImage->Write("/threshold.bmp");
	filteredImage = thresholdImage->ParticleFilter(criteria, 1);	//Remove small particles
	delete thresholdImage;
...
/**
 * s a score (0-100) estimating how rectangular the particle is by comparing the area of the particle
